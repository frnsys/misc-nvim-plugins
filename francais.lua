...
---   apt install dictd dict-freedict-eng-fra dict-freedict-fra-eng
---
--- Install the conjugator:
---   apt install verbiste
---
...
---   LanguageTool (https://languagetool.org/; tested with 6.1)
---     wget https://languagetool.org/download/LanguageTool-6.1.zip
---
...
local function run_languagetool(path, diagnostics)
  execute({
    ...
    if #diagnostics == 0 then
      show_float("Succès!", {"Aucune erreur trouvée."})
    else
      vim.diagnostic.set(ns, 0, diagnostics)
    end
  end, function(_, err)
...
local function lookup_en(word)
  local output_d = vim.fn.systemlist({
    'dict', '-d',
    'fd-eng-fra', word
  })

  table.insert(output_d,
    '--------------------')

  local output_t = vim.fn.systemlist({
    'trans',
    '-s', 'en',
    '-t', 'fr',
    '--no-ansi', word
  ...
  local lines = concat(output_d, output_t)
  show_float('Recherche Anglais', lines)
end
...
local function lookup_fr(word)
  ...
  local lines = concat(output_d, output_t)
  show_float('Recherche Francis', lines)
end
...
local function deconjugate(verb)
  execute({
    ...
    show_float('Deconjugaison', output)
  end, function(_, output)
    show_float('Deconjugaison', output)
  end)
end
...
--- Translate the line under cursor (fr -> en).
...
local function show_help()
  local lines = {
    '<leader>g\tVoir les fautes de grammaire.',
    '<leader>n\tRechercher un mot anglais.',
    '<leader>r\tRechercher un mot francais.',
    '<leader>c\tConjuguer un verbe.',
    '<leader>d\tDeconjuguer un verbe.',
    '<leader>x\tTraduire la ligne courante.',
  }
  show_float('Aide', lines)
end
...
vim.api.nvim_create_autocmd({'BufRead', 'BufNewFile'}, {
  ...
  callback = function(opts)
    ...
    --- User commands
    vim.api.nvim_buf_create_user_command(0, 'Aide', show_help, {})
